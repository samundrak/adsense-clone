<h1>Your Earned credits details</h1>
<?php 
session_start();
...
$query = mysql_query("SELECT sum(credit_earn) FROM credits_ WHERE  username = '$user' AND date='$datess'");
$fetch = mysql_fetch_array($query);
$today = $fetch[0];
$yquery = mysql_query("SELECT sum(credit_earn) FROM credits_ WHERE username = '$user' AND date = '$echo'");
...
$total_query = mysql_query("SELECT credits_earned from final_data where username = '$user' and date= '$datess'");
$total_fetch = mysql_fetch_assoc($total_query);
$total = $total_fetch['credits_earned'];
...
You have earned one credit from the click of website $pwebsite_ on $pdate_ at $ptime_.
</i></div><hr/>";
}?></div>
